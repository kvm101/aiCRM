...
public class ClientControllerTest {
  ...
  void getAllClients_ShouldReturnOk() throws Exception {
    ClientDTO dto =
        new ClientDTO(1L, "Test Name", "Company", "email@test.com", "123", null, null, null);
    when(clientService.getAllClients(1L, null)).thenReturn(List.of(dto));

    mockMvc
        .perform(get("/clients/filtered").header("X-Project-Id", "1"))
  ...
  void createClient_ShouldReturnCreated() throws Exception {
    ClientDTO dto =
        new ClientDTO(1L, "New Client", "Company", "email@test.com", "123", null, null, null);
  ...
  void readClient_WhenFound_ShouldReturnOk() throws Exception {
    ClientDTO dto =
        new ClientDTO(10L, "Found Client", "Company", "email@test.com", "123", null, null, null);
    when(clientService.getClientById(1L, 10L)).thenReturn(dto);

    mockMvc
        .perform(get("/clients/10").header("X-Project-Id", "1"))
        .andExpect(status().isOk())
