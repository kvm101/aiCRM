...
public class TaskController {
  ...
  public ResponseEntity<List<TaskDTO>> getTasksBeforeDeadline(
    ...
    List<TaskDTO> tasks =
        taskService.getTasksBeforeDeadline(projectId, deadline).stream()
            .map(this::entityToDto)
            .collect(Collectors.toList());
    return ResponseEntity.ok(tasks);
  }
  ...
  public ResponseEntity<TaskDTO> updateTask(
    ...
    Task task =
        taskService.updateTask(
            projectId, id, updatedTask, taskDTO.getDealId(), taskDTO.getClientId());
    return ResponseEntity.ok(entityToDto(task));
  }

  @DeleteMapping("/{id}")
  public ResponseEntity<Void> deleteTask(
      @PathVariable Long id, @RequestHeader(name = "X-Project-Id") String projectIdStr) {
  ...
  private Task dtoToEntity(TaskDTO dto) {
    ...
    // Note: dealId mapping requires DealRepository, but we can skip it for update/create for now if
    // not sent,
    // or let TaskService handle Deal attachment. Currently we only create tasks with Deal attached
    // from somewhere else.
    return task;
  }
