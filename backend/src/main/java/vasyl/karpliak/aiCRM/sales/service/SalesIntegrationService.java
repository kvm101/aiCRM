...
public class SalesIntegrationService {
  ...
  private final vasyl.karpliak.aiCRM.communications.repository.NotificationRepository
  ...
  public SalesIntegrationService(
      ...
      vasyl.karpliak.aiCRM.communications.repository.NotificationRepository
          notificationRepository) {
  ...
  public void processIncomingMessage(
      Long userId, String contactIdentifier, String contactName, String text) {
    User user =
        userRepository
            .findById(userId)
            .orElseThrow(() -> new RuntimeException("Користувача не знайдено"));

    // Try to find client by email (which acts as our contact identifier for both email and facebook
    // identifier)
    Optional<Client> existingClient =
        user.getClients().stream()
            .filter(
                c ->
                    c.getEmail().equalsIgnoreCase(contactIdentifier)
                        || c.getName().equalsIgnoreCase(contactIdentifier))
            .findFirst();
    ...
    if (existingClient.isPresent()) {
      ...
      client.setName(
          contactName != null && !contactName.isBlank() ? contactName : contactIdentifier);
      client.setEmail(contactIdentifier);
      client.setPhone("");
      client.setCompany("");
      client.setStatus(ClientStatus.NEW);
      vasyl.karpliak.aiCRM.sales.dto.ClientDTO newDto =
          clientService.createClient(client, 1L); // Default project for incoming external messages
      clientId = newDto.id();
    ...
    List<Deal> activeDeals =
        dealService.getAllDeals(1L).stream()
    ...
    if (activeDeals.isEmpty()) {
      ...
      DealCreateRequest request =
          new DealCreateRequest("Угода з " + finalClientName, BigDecimal.ZERO, "USD", clientId);
    ...
    // Create notification
    vasyl.karpliak.aiCRM.communications.domain.Notification notification =
        new vasyl.karpliak.aiCRM.communications.domain.Notification();
