...
@Component
public final class SearchAITools {

  private static final Logger log = LoggerFactory.getLogger(SearchAITools.class);
  private final SemanticSearchService searchService;

  public SearchAITools(
      org.springframework.beans.factory.ObjectProvider<SemanticSearchService>
  ...
  @Tool(
      description =
          "Виконує семантичний пошук по базі знань (векторній базі) за текстовим запитом. Використовуй це, коли користувач просить знайти інформацію в документах, нотатках або історії.")
  public String searchKnowledgeBase(String query) {
    ...
    StringBuilder result =
        new StringBuilder("Знайдено таку інформацію (в порядку релевантності):\n");
    response
        .hits()
        .forEach(
            hit -> {
