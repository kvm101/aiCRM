...
public class MessageDispatcherService {
  ...
  public void processInboundMessage(UnifiedMessage unifiedMessage) {
    ...
    Optional<ChatSession> optionalSession =
        chatSessionRepository.findByExternalChatIdAndChannelTypeAndTeamId(
            unifiedMessage.externalChatId(), unifiedMessage.channel(), unifiedMessage.teamId());
    ...
    if (optionalSession.isPresent() && optionalSession.get().getStatus() == SessionStatus.OPEN) {
      ...
      vasyl.karpliak.aiCRM.iam.domain.Project defaultProject =
    ...
    eventPublisher.publishEvent(
        new InboundMessageEvent(this, message, session.getAssignedUserId()));
  }
}
