...
/**
 * Сервіс для отримання вхідних email через IMAP. Перевіряє поштову скриньку кожні 60 секунд,
 * зберігає нові листи в БД та створює нотифікації для користувача.
 */
@Service
public class InboundEmailService {
    ...
    try {
      ...
      try {
        ...
        java.util.Set<String> existingIds =
            emailMessageRepository.findByExternalMessageIdIn(messageIds).stream()
  ...
  private String extractFromMultipart(MimeMultipart multipart)
