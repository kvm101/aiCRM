...
public class NotificationController {
  ...
  public ResponseEntity<List<NotificationDto>> getUserNotifications(
    ...
    List<NotificationDto> notifications =
        notificationRepository.findByUserIdOrderByCreatedAtDesc(getUserId(userId)).stream()
            .map(this::mapToDto)
  ...
  public ResponseEntity<List<NotificationDto>> getUnreadNotifications(
    ...
    if (userId == null) return ResponseEntity.ok(List.of());
    List<NotificationDto> notifications =
        notificationRepository.findByUserIdAndReadFalse(getUserId(userId)).stream()
            .map(this::mapToDto)
            .collect(Collectors.toList());
    return ResponseEntity.ok(notifications);
  }
  ...
  public ResponseEntity<Void> markAsRead(
      @PathVariable Long id, @RequestHeader(name = "X-User-Id", required = false) String userId) {
