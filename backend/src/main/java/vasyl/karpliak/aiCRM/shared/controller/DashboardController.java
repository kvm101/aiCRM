...
public class DashboardController {
  ...
  public ResponseEntity<Map<String, Object>> getStats(
    ...
    if (projectIdStr == null || projectIdStr.isBlank()) {
      projectId = projectRepository.findAll().stream().findFirst().map(p -> p.getId()).orElse(null);
    } else {
    ...
    long activeDeals =
        dealRepository.countByProjectIdAndStatus(projectId, DealStatus.NEW)
    ...
    long unreadMessages =
        chatSessionRepository.findByProjectId(projectId).stream()
