...
public class MessageOutboundListener {
    ...
    public MessageOutboundListener(BotRepository botRepository,
    ...
    public void processOutboundMessage(UnifiedMessage unifiedMessage) {
        ...
        if (adapter == null) {
            throw new IllegalArgumentException("Не знайдено адаптер для каналу: " + unifiedMessage.channel());
        }

        // Шукаємо токен бота для цієї команди та каналу
        Optional<Bot> optionalBot = botRepository.findByTeamIdAndChannelType(unifiedMessage.teamId(), unifiedMessage.channel());
        
        if (optionalBot.isEmpty()) {
            throw new IllegalStateException("Бота не знайдено для team: " + unifiedMessage.teamId() + " та каналу: " + unifiedMessage.channel());
        }
