...
public class DealController {
    ...
    public ResponseEntity<List<DealResponse>> getAllDeals(
        ...
        List<DealResponse> deals = dealService.getAllDeals(resolveProjectId(projectId))
                .stream()
    ...
    public ResponseEntity<List<vasyl.karpliak.aiCRM.sales.dto.DealEventDto>> getDealEvents(@PathVariable Long id, @RequestHeader(name = "X-Project-Id") String projectId) {
    ...
    public ResponseEntity<vasyl.karpliak.aiCRM.sales.dto.DealEventDto> addDealNote(
        ...
        return ResponseEntity.ok(dealService.addNoteToDeal(getProjectId(projectId), id, payload.get("text")));
    }

    @PostMapping
    public ResponseEntity<DealResponse> createDeal(@RequestBody DealCreateRequest request, @RequestHeader(name = "X-Project-Id") String projectId, @RequestHeader(name = "X-User-Id") String userId) {
        return ResponseEntity.ok(mapToResponse(dealService.createDeal(getProjectId(projectId), Long.parseLong(userId), request)));
    }

    @PatchMapping("/{id}")
    public ResponseEntity<DealResponse> updateDeal(@PathVariable Long id, @RequestBody DealUpdateRequest request, @RequestHeader(name = "X-Project-Id") String projectId) {
        return ResponseEntity.ok(mapToResponse(dealService.updateDeal(getProjectId(projectId), id, request)));
    }

    @PatchMapping("/{id}/status")
    public ResponseEntity<DealResponse> changeStatus(@PathVariable Long id, @RequestParam DealStatus status, @RequestHeader(name = "X-Project-Id") String projectId) {
        return ResponseEntity.ok(mapToResponse(dealService.changeStatus(getProjectId(projectId), id, status)));
    }

    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deleteDeal(@PathVariable Long id, @RequestHeader(name = "X-Project-Id") String projectId) {
